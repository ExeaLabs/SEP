...
def load_iucn_data(iucn_csv: str | None = None) -> pd.DataFrame:
    ...
    if iucn_csv is None:
        iucn_csv = _find_downloaded_csv([
            "*iucn*.csv", "*IUCN*.csv", "*extinction*.csv", "*threat*.csv",
        ])
...
def load_gbif_data(gbif_csv: str | None = None) -> pd.DataFrame:
    """Load GBIF occurrence records with coordinates."""
    if gbif_csv is None:
        gbif_csv = _find_downloaded_csv([
            "*gbif*.csv", "*GBIF*.csv", "*occurrence*.csv",
        ])
    ...
    df = pd.read_csv(gbif_csv, low_memory=False)
    name_col = _find_column(df, IUCN_NAME_COLS + ["species", "scientificName"])
    ...
    if name_col is None or lat_col is None or lon_col is None:
        raise ValueError(
            f"Could not detect name/lat/lon columns in {gbif_csv}. "
            f"Columns found: {list(df.columns)}"
        )
    ...
    print(f"Loaded {len(df)} GBIF species locations from {gbif_csv}")
    return df
